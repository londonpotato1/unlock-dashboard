...
const unlocks = [
  ...
  },
  {
    ...
    unlockPct: 12.79, unlockVal: 0.895, recipient: "5개 카테고리 동시 배출 — Private Sale 53% · Team 16% · Community Rewards 16% · Ecosystem 10% · BD 5%",
  ...
  },
  {
    ...
    unlockPct: 4.0, unlockVal: 2.33, recipient: "Core Contributors (linear vesting 회차)",
    circulating: "2.12B", totalSupply: "4.2B (Genesis)", maxSupply: "5.67B (인플레이션 포함, 디스인플레이션 모델)",
    featured: true,
    allocation: [
    ...
    market: { price: "$0.02747", mcap: "$57.1M", fdv: "$118.79M", vol24h: "$20.1M", athDrop: "-96.2% (ATH $0.7505, 2024-12-10)", exchanges: "Bithumb(KRW), Gate.io, Bitvavo, Bybit, Binance Futures (현물 미상장)" },
    vesting: "▶ 토큰이 뭐야? peaq는 DePIN(분산 물리 인프라 — 로봇·차량·IoT 기기가 자체 ID와 지갑을 갖고 온체인 정산하는 구조) 전용 L1(레이어1 — 자체 메인넷). Machine Economy(머신 이코노미 — 사람이 아닌 기계가 거래 주체)를 표방하며 Machine Layer + Trust Layer + Service Layer 3계층 구조. Genesis 총공급 4.2B, 인플레이션 포함 최대 5.67B. 현재 유통량 약 2.12~2.18B(Tokenomist vs CoinGecko 시점 차이, 50~52% 진행). ▶ 무슨 이벤트? 2026-06-12(UTC)에 84.84M PEAQ가 Core Contributors(핵심 개발팀) 카테고리에서 배출되는 linear vesting(선형 분할 — 9~36개월 일정 페이스로 매월 풀림) 회차. cliff(절벽 해제) 단발 이벤트 아님. ▶ 얼마나 큰 규모? 총공급 4.2B의 약 2.0% / 현재 유통량 기준 약 4.0%(Tokenomist 2.12B 기준) 또는 약 3.9%(CoinGecko 2.18B 기준) / 달러 환산 약 $2.33M(현재가 $0.02747 기준). 24h 거래량 $20M 대비 약 11.7% 수준 — 거래량으로는 흡수 가능한 규모. ▶ 누가 받음? Core Contributors(핵심 개발팀, 전체 15.77%)가 단독 수령. 직접 매도 가능성보다는 운영비·그랜트(grant — 빌더 보조금)·라이센싱 등으로 분배될 가능성이 큰 카테고리지만, 일부 수령자가 즉시 매도하는 패턴은 항상 잠재. ▶ 🇰🇷 한국 일정은? 사용자 제보 기준 6/12 unlock 직전인 6/11까지 한국 텔레그램 AMA(Ask-Me-Anything) 등 community 이벤트 일정이 잡혀 있다는 정보 — 텔레그램 채널 기반, peaq 공식 트위터/블로그에서는 별도 공지 미확인 상태이므로 일정 정확도는 채널 직접 확인 권장. catalysts(가격 호재 — 이벤트가 가격을 미는 동력) 누적 가능성 있음. ▶ 시장 미리 반응? 지난 7일 +57.6%, 24h +28.4% 강한 상승 진행 중. MastChain(peaq 위 해양 DePIN 프로젝트) MastNode 출하 일정 진행 중(정확한 수량·일정은 peaq 공식 채널 확인 권장) + Bithumb 신규 KRW 상장(VANA 동시) 등 catalyst 후보가 겹친 상태 (개별 catalyst 의 가격 영향은 공식 확인 후 판단 권장). ATL $0.0122(2026-03-15) 대비 현재 $0.02747 = +125% 반등. ▶ 시장 상황은? MCap $57.1M / FDV $118.79M (CoinGecko 기준) = 약 2.1배 — 잠재 희석 압력 중간 수준. ATH $0.7505(2024-12-10) 대비 -96.2%로 깊은 약세 구간이지만 단기 30일 강한 반등. Binance Futures(PEAQUSDT) OI는 $1.93M로 무난한 수준이라 강제청산 리스크 낮음. ▶ 한국에서 팔 수 있나? Bithumb PEAQ/KRW 상장으로 한국 직접 매도 가능. Upbit 미상장. Binance 현물 미상장(Futures만), 해외 현물은 Gate.io·Bitvavo·Bybit이 주력. ▶ 데이터 소스: Tokenomist(unlock 일자·수량·카테고리) + CoinGecko(시장 데이터) + peaq 공식 docs.peaq.xyz(allocation) + CoinGlass(Futures OI) 크로스체크.",
    strategy: [
      { type: "caution", title: "7일 +57.6% 급등 후 6/12 unlock — sell-the-news 위험 우선 인식",
        desc: "지난 7일 +57.6%, 24h +28.4% 강한 상승 진행 중인 상태에서 6/12 unlock(84.84M = 유통량 약 4.0%, $2.33M)이 예정. 급등 후 이벤트가 발생하면 sell-the-news(호재 후 차익 실현 — 기대감으로 미리 산 자금이 이벤트 발생 시 일제히 매도) 패턴 위험이 가장 직접적 시나리오. Core Contributors 카테고리 자체는 즉시 매도보다 운영비·그랜트 분배 비중이 크지만, 차익 실현 매물과 합쳐지면 단기 압력 발생 가능. 사용자 제보 기준 6/11까지 한국 텔레그램 AMA 등 community 일정이 있다고 알려졌으나 peaq 공식 채널(트위터·블로그·docs)에서는 별도 공지 미확인 상태 — catalyst로 가정하기 전 공식 출처 직접 확인 권장.",
