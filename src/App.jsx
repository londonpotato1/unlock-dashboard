...
const unlocks = [
  ...
  },
  {
    ...
    unlockPct: 1.92, unlockVal: 4.6, recipient: "Future Airdrop final cliff + Linea Consortium + Long-Term Ecosystem Fund",
    circulating: "24.91B", totalSupply: "72.01B", maxSupply: "72.01B",
    allocation: [
      { name: "Long-Term Ecosystem Fund", pct: 50.0 },
      { name: "Linea Consortium (Ecosystem Activation)", pct: 16.0 },
      { name: "Consensys Treasury (5년 cliff, 0% 진행)", pct: 15.0 },
      { name: "Early Contributor Airdrop & LPs", pct: 14.0 },
      { name: "Future Airdrop", pct: 5.0 },
    ],
    unlockBreakdown: [
      { name: "Future Airdrop (final cliff, 100% 완료)", amount: "~720M", pct: 52.2, color: "#ef4444" },
      { name: "Linea Consortium (12m linear 8회차)", amount: "480.07M", pct: 34.8, color: "#3b82f6" },
      { name: "Long-Term Ecosystem Fund (119m decaying 8회차)", amount: "~300M", pct: 13.0, color: "#8b5cf6" },
    ],
    market: { price: "$0.0034", mcap: "$85M", fdv: "$245M", vol24h: "N/A", athDrop: "-92.7%", exchanges: "Binance, Coinbase, OKX, Bybit, Upbit, Bithumb, HTX, Kraken" },
    vesting: "▶ 토큰이 뭐야? LINEA는 Consensys(메타마스크 모회사)가 개발한 L2(레이어2 — 이더리움 위 확장 체인) 토큰. ZK(영지식증명) 기반 zkEVM 롤업으로 ETH를 가스 토큰으로 사용하며 dual-burn(이중 소각) 메커니즘 채택. governance(거버넌스) 토큰 아님 — DAO(탈중앙 조직) 없이 Linea Consortium(ENS Labs·Eigen Labs·SharpLink·Status·Consensys 5개 Ethereum stewards) 협의체 운영. 총공급량 72.01B, TGE 2025-09-10. ▶ 무슨 이벤트? 2026-04-10은 매월 정기 unlock의 8회차이자 Future Airdrop 5%(3.6B) 카테고리의 최종 cliff 완료 시점. 세 카테고리에서 동시에 풀린 이벤트 — ① Future Airdrop 마지막 분량 ~720M(3.6B를 5개월 linear vesting으로 분배한 마지막 회차 추정, DropsTab '100% Unlocked on 10 Apr 2026' 표기), ② Linea Consortium(=Ecosystem Activation, 전체 16%) 480.07M 매월 8회차, ③ Long-Term Ecosystem Fund(=Long-term Alignment, 전체 50%) ~300M 매월 8회차(decaying linear). ▶ 얼마나 큰 규모? 합산 약 1.38B LINEA = 총공급량 72.01B의 1.92% / 유통량 24.91B의 5.53% / 달러 환산 약 $4.6M. CoinGecko Tokenomist 위젯도 동일하게 ~1.38B로 표기. 유통량 대비 5.53%는 단기 매도 압력으로 작지 않은 규모. MCap $85M, FDV $245M, ATH 대비 -92.7%. ▶ ⚠️ 정정 노트 — 'Consensys Treasury 수령'은 부정확! 4/10 unlock에는 Consensys Treasury(전체 15%, 10.8B)가 포함되지 않음. Consensys Treasury는 5년 완전 cliff 적용 — DropsTab/Tokenomist 모두 0% 진행, 1587일 남음(2030년 9월까지 0)으로 일관 표기. 따라서 실제 수령자는 위 세 카테고리(Future Airdrop final + Linea Consortium + Long-Term Ecosystem Fund). ▶ 누가 실제로 받음? Linea Consortium 5개 Ethereum stewards가 관리하는 펀드 버킷이 수령. 즉각 시장 매도가 아니라 ① CEX·LP 유동성 공급, ② 빌더 그랜트(grant — 보조금), ③ Ethereum public goods(이더리움 공공재) 자금 조달 용도로 분배. 다만 Future Airdrop 카테고리는 사용자(early users)에게 직접 배포된 만큼 일부는 시장에 즉시 도달했을 가능성. ▶ 시장 미리 반응? 4/10 시점 vol24h 데이터 미수집. ATH 대비 -92.7% 상태로 깊은 약세 구간이었음. 매월 반복되는 정기 이벤트 cadence(Sep TGE → Oct → Nov → Dec → ... → Apr 8회차 → May 9회차)로 시장이 어느 정도 priced in(미리 반영)된 상태. 다만 Future Airdrop 마지막 cliff까지 겹치면서 4월 unlock은 5월 unlock(780M)보다 더 큰 이벤트였음. ▶ 한국에서 팔 수 있나? Bithumb·Upbit 양쪽 LINEA/KRW 페어 활성. Upbit가 주력 KRW 채널($1M+/24h). 해외 거래소 Binance·Coinbase·OKX·Bybit 등도 충분한 유동성 제공.",
  },
  {
  ...
  {
    date: "2026-05-10", token: "LINEA", name: "Linea", cat: "L2 (Consensys)",
    unlockPct: 1.08, unlockVal: 3.03, recipient: "Linea Consortium + Long-Term Ecosystem Fund (월간 정기)",
    circulating: "24.91B", totalSupply: "72.01B", maxSupply: "72.01B",
    allocation: [
      { name: "Long-Term Ecosystem Fund", pct: 50.0 },
      { name: "Linea Consortium (Ecosystem Activation)", pct: 16.0 },
      { name: "Consensys Treasury (5년 cliff)", pct: 15.0 },
      { name: "Early Contributor Airdrop & LPs", pct: 14.0 },
      { name: "Future Airdrop", pct: 5.0 },
    ],
    unlockBreakdown: [
      { name: "Linea Consortium (12m linear)", amount: "480.07M", pct: 61.5, color: "#3b82f6" },
      { name: "Long-Term Ecosystem Fund (119m decaying)", amount: "300.08M", pct: 38.5, color: "#8b5cf6" },
    ],
    market: { price: "$0.00387", mcap: "$96.5M", fdv: "$278.8M", vol24h: "$22M", athDrop: "-91.7%", exchanges: "Binance, Coinbase, OKX, Bybit, Upbit, Bithumb, HTX, Kraken, Bitget" },
    vesting: "▶ 토큰이 뭐야? LINEA는 Consensys(메타마스크 모회사)가 개발한 L2(레이어2 — 이더리움 위 확장 체인) 토큰. ZK(영지식증명) 기반 zkEVM 롤업으로 ETH를 가스 토큰으로 사용하며 dual-burn(이중 소각 — L2 수수료의 80%로 LINEA, 20%로 ETH 소각) 메커니즘 채택. governance(거버넌스) 토큰 아님 — DAO(탈중앙 조직) 없이 Linea Consortium(ENS Labs·Eigen Labs·SharpLink·Status·Consensys 5개 Ethereum stewards — 이더리움 생태계 관리자) 협의체가 운영. Binance·Coinbase·OKX·Bybit·Upbit·Bithumb 상장. 총공급량 72.01B, 현재 유통량 24.91B(34.6% 진행). ▶ 무슨 이벤트? 2026-05-10에 780.15M LINEA가 두 카테고리에서 동시 배출되는 정기 월간 이벤트. ① Linea Consortium(=공식 Ecosystem Activation, 전체 16%) 480.07M = 12개월 linear vesting(선형 분할 — 매월 일정량씩 풀림) 8회차로 79.17%까지 진행. ② Long-Term Ecosystem Fund(=공식 Long-Term Alignment, 전체 50%) 300.08M = 119개월 decaying linear emission(점감 선형 발행 — 초반 많이→갈수록 적게) 8회차로 5.82%까지 진행. ▶ 얼마나 큰 규모? 합산 780.15M LINEA = 총공급량 72.01B의 1.08% / 유통량 24.91B의 3.13% / 달러 환산 $3.03M. 24h 거래량 $22M(CoinGecko 통합 기준) 대비 약 14% 수준 — 시장이 흡수 가능한 규모. MCap $96.5M, FDV $278.8M으로 FDV/MCap 비율 약 2.9배. ▶ 누가 받음? Linea Consortium 5개 Ethereum stewards(ENS Labs·Eigen Labs·SharpLink·Status·Consensys)가 관리하는 두 펀드 버킷이 수령. 즉각 시장 매도가 아니라 ① CEX·LP 유동성 공급, ② 빌더 그랜트(grant — 보조금), ③ Ethereum public goods(이더리움 공공재) 자금 조달 용도로 분배 예정. 다만 그랜트 수령자(빌더·프로젝트)가 즉시 매도할 가능성은 항상 잠재. ▶ Consensys Treasury는 어디 갔어? 별도 카테고리(전체 15%, 10.8B LINEA)로 5년 완전 cliff(절벽 해제 — 잠겨 있던 물량이 한 번에 풀림) 적용. 2030년 9월까지 0% 진행 상태로 이번 5/10 unlock에는 포함되지 않음. ▶ 데이터 소스 간 drift 주의! CoinGecko(Tokenomist 위젯)는 5/10 unlock을 1.38B / $5.35M / 1.9%로 더 크게 표시(Future Airdrop 300M + Consortium-Ignition 480M + Consortium-Long term 600M 3그룹). 그러나 Future Airdrop 5%(3.60B)는 DropsTab 기준 4/10에 100% unlocked 완료된 카테고리이므로 CoinGecko 표기는 stale(오래된) 데이터로 추정. 본 entry는 DropsTab + 텔레그램 봇 스크린샷이 일치하는 780.15M 기준으로 작성. 실제 unlock 당일 모니터링 필수. ▶ 시장 미리 반응? 현재가 $0.00387, 7일 +5.87% / 30일 +20.08% / 1년 -89.11%. ATH 대비 -91.7%로 깊은 약세 구간이지만 30일 단기 반등 진행 중이며 DropsTab sentiment(시장 심리 지표) 'Bullish'. 5/6 Linea가 ZK 롤업 스택을 Linux Foundation 오픈소스 그룹에 기여한다는 발표로 +3.8% 상승. 매월 반복되는 정기 이벤트라 선반영(priced in — 시장이 미리 가격에 반영) 경향이 강해 단발성 충격은 제한적. ▶ 한국에서 팔 수 있나? Upbit·Bithumb 양쪽 상장 확인. Upbit LINEA/KRW $1.15M/24h, Bithumb LINEA/KRW $191K/24h로 Upbit가 주력 채널. 한국 KRW 직접 매도 가능. 해외 거래소(Binance·Coinbase·OKX·Bybit·HTX 등)도 충분한 유동성 제공.",
  },
  {
  ...
  {
    date: "2026-05-17", token: "SOLV", name: "Solv Protocol", cat: "BTCFi",
    unlockPct: 12.79, unlockVal: 0.86, recipient: "5개 카테고리 동시 배출 — Private Sale 53% · Team 16% · Community Rewards 16% · Ecosystem 10% · BD 5%",
    circulating: "1.48B", totalSupply: "8.4B", maxSupply: "9.66B (거버넌스로 변경 가능)",
    allocation: [
      { name: "Private Sale Investors", pct: 25.10 },
      { name: "Community Rewards & DAO", pct: 15.65 },
      { name: "Bitcoin Reserve Offering", pct: 13.04 },
      { name: "Team & Advisors", pct: 11.30 },
      { name: "Ecosystem Development", pct: 7.08 },
      { name: "Business Development", pct: 3.48 },
      { name: "Binance Megadrop · IDO · 유동성 · 기타", pct: 24.35 },
    ],
    unlockBreakdown: [
      { name: "Private Sale Investors", amount: "101.02M", pct: 53.3, color: "#ef4444" },
      { name: "Team & Advisors", amount: "30.32M", pct: 16.0, color: "#f59e0b" },
      { name: "Community Rewards (External Partners)", amount: "30.34M", pct: 16.0, color: "#3b82f6" },
      { name: "Ecosystem Development", amount: "18.18M", pct: 9.6, color: "#8b5cf6" },
      { name: "Business Development", amount: "9.80M", pct: 5.1, color: "#10b981" },
    ],
    market: { price: "$0.00454", mcap: "$6.73M", fdv: "$43.83M (최대공급 9.66B 기준)", vol24h: "$7.6~10.1M", athDrop: "-97.7% (ATH $0.2001, 2025-01-17)", exchanges: "Binance, Bybit, Gate, KuCoin, Bithumb" },
    vesting: "▶ 토큰이 뭐야? SOLV는 Solv Protocol의 BTCFi(비트코인 기반 DeFi — BTC 담보 토큰화·LST·수익 상품) 인프라 토큰. SolvBTC(BTC 백업 LST — liquid staking token, BTC를 예치하면 받는 유동성 증명) 발행 + 멀티체인 배포(BNB Chain, Ethereum, Solana, Arbitrum, Avalanche, Base, Berachain). TGE(토큰 발행) 2025-01-17, Binance 스팟 동시 상장. ▶ 무슨 이벤트? 2026-05-17(UTC)에 189.67M SOLV가 5개 카테고리에서 동시 배출되는 monthly linear(월간 선형 — 매월 일정량씩 풀림) 회차. cliff(절벽 해제 — 잠겨 있던 물량이 한 번에 풀림) 단일 이벤트 아님. ▶ 얼마나 큰 규모? 최대공급 9.66B 기준 1.96% (= 총공급 8.4B 기준 약 2.26%) / 현재 유통량 1.48B의 12.79% / 달러 환산 약 $859K(2026-05-15 기준 가격 $0.00454, DropsTab 최신값). 24h 거래량 $7.6~10.1M 대비 약 8~11% 수준 — 거래량 대비 비율로는 시장이 흡수 가능. 단, MCap이 $6.7M에 불과해 절대 규모는 작지만 시총 대비 비율로는 부담. ▶ 5개 카테고리 동시 배출 = 어느 카테고리? Private Sale Investors 101.02M(53.3%) + Team & Advisors 30.32M(16.0%) + Community Rewards(External Partners) 30.34M(16.0%) + Ecosystem Development 18.18M(9.6%) + Business Development 9.80M(5.2%). 내부자 비중(Private + Team) 합산 69.3% — 매도 압력 가능성 높은 트랜치가 다수. ▶ ⚠️ '41%' 헷갈리지 말 것! 텔레그램·Tokenomist·DropsTab에서 본 '41.67%'는 Business Development 카테고리의 누적 unlock 진행률(=해당 카테고리 총 할당량 중 이미 풀린 비율)이지 이번 5/17 언락 규모가 아님. 실제 unlock 규모는 최대공급 기준 1.96% / 총공급 기준 2.26% / MCap 대비 12.79%. ▶ 시장 상황은? ATH $0.2001(2025-01-17 TGE 직후) 대비 현재 $0.00472 = -97.7%로 역대 최저권. CertiK 감사 이력 있음(상세 점수는 certik.com 직접 확인 권장), 과거 스마트컨트랙트 exploit 보도 이력 — 공식 사후 보고서 미확인 시 추가 검증 필요. MCap $6.73M / FDV $43.83M(최대공급 기준) = 약 6.5배 — 잠재 희석 압력 매우 큼. 최대공급(9.66B) 기준 미유통·미발행 포함 약 84.7%(8.18B SOLV)가 여전히 잠긴/미발행 상태로 향후 지속적 매도 압력 누적 구조. ▶ 한국에서 팔 수 있나? Bithumb 상장 보도 있으나 KRW 마켓 활성 여부 재확인 필요. Upbit 미상장. 해외는 Binance·Bybit·Gate·KuCoin이 주력 채널이며 현물 유동성은 Binance에 집중. ▶ 데이터 소스: DropsTab(189.67M / 1.96% / $895K, 카테고리 5개 분배) + Tokenomist(allocation 구조) + CoinGecko(현재가/MCap/FDV) 크로스체크. CryptoRank 일부 화면에는 다른 날짜·수치가 표시되는 경우 있음 — 무시.",
    strategy: [
      { type: "caution", title: "이미 -97.7% 가격 + 작은 절대 규모 = 단순 숏보다는 신중 접근",
        desc: "5/17 언락 규모는 $859K로 절대 금액이 작지만, MCap $6.73M 대비 12.79% 비율은 부담. 내부자(Private Sale + Team) 비중이 회차의 69.3%로 매도 가능성 높은 트랜치 다수. 그러나 가격이 이미 ATH 대비 -97.7%로 추가 하방 여지가 제한적이며, 거래량/유동성도 얕아 큰 숏 포지션 진입 시 비대칭 위험. ▶ 관전 포인트: 빗썸 KRW 마켓 활성 여부 확인(거래 가능 채널 협소), 가격이 이미 바닥권이라 squeeze(숏스퀴즈 — 가격이 반대로 튀며 숏 포지션 강제청산) 위험 인지 필요.",
        risk: "유동성 얕은 마이크로캡 토큰의 일반 위험: ① 호가 깊이 부족으로 슬리피지(주문이 호가를 밀어내며 불리하게 체결) 큼, ② 거래량 변동성 극심, ③ exploit 이력으로 추가 보안 사고 시 양방향 급변 가능. 최대공급 기준 약 84.7%가 미유통 상태라 향후 다른 회차에서도 유사 압력 반복 — 단발성 이벤트로 접근 권장." },
    ],
  },
  {
  ...
  {
    date: "2026-06-12", token: "PEAQ", name: "peaq network", cat: "DePIN L1 (Machine Economy)",
    unlockPct: 4.0, unlockVal: 2.36, recipient: "Core Contributors (linear vesting 회차)",
    circulating: "2.12B", totalSupply: "4.4B (현재, Genesis 4.2B + 인플레이션 누적)", maxSupply: "5.67B (인플레이션 포함, 디스인플레이션 모델)",
    featured: true,
    allocation: [
      { name: "Investors", pct: 26.82 },
      { name: "Inflation (Emissions, 연 3.5% → 점진 감소)", pct: 21.13 },
      { name: "Community", pct: 16.56 },
      { name: "Ecosystem & Treasury", pct: 15.77 },
      { name: "Core Contributors", pct: 15.77 },
      { name: "Network Security", pct: 3.94 },
    ],
    market: { price: "$0.02781", mcap: "$57.7~58.1M", fdv: "$120.9M(CG, 현재 total supply 기준) / $154.5M(CMC, max supply 기준)", vol24h: "$13.4~32.4M(소스별 차이)", athDrop: "-96.3% (ATH $0.7505, 2024-12-10)", exchanges: "Bithumb(KRW), Gate.io, Bitvavo, Bybit, Binance Futures (현물 미상장)" },
    vesting: "▶ 토큰이 뭐야? peaq는 DePIN(분산 물리 인프라 — 로봇·차량·IoT 기기가 자체 ID와 지갑을 갖고 온체인 정산하는 구조) 전용 L1(레이어1 — 자체 메인넷). Machine Economy(머신 이코노미 — 사람이 아닌 기계가 거래 주체)를 표방하며 Machine Layer + Trust Layer + Service Layer 3계층 구조. Genesis 총공급 4.2B + 인플레이션 누적분 약 0.2B = 현재 total supply 약 4.4B, 디스인플레이션 종료 시 최대 5.67B. 현재 유통량 약 2.12B(전체 max supply의 37.4%). ▶ 무슨 이벤트? 2026-06-12(UTC)에 84.84M PEAQ가 Core Contributors(핵심 개발팀) 카테고리에서 배출되는 linear vesting(선형 분할 — 9~36개월 일정 페이스로 매월 풀림) 회차. cliff(절벽 해제) 단발 이벤트 아님. ▶ 얼마나 큰 규모? 현재 total supply 4.4B 기준 약 1.93% / max supply 5.67B 기준 약 1.50% / 현재 유통량 2.12B 기준 약 4.0% / 달러 환산 약 $2.36M(2026-05-15 기준 가격 $0.02781). 24h 거래량 $20M 대비 약 11.7% 수준 — 거래량으로는 흡수 가능한 규모. ▶ 누가 받음? Core Contributors(핵심 개발팀, 전체 15.77%)가 단독 수령. 직접 매도 가능성보다는 운영비·그랜트(grant — 빌더 보조금)·라이센싱 등으로 분배될 가능성이 큰 카테고리지만, 일부 수령자가 즉시 매도하는 패턴은 항상 잠재. ▶ 🇰🇷 한국 일정은? 사용자 제보 기준 6/12 unlock 직전인 6/11까지 한국 텔레그램 AMA(Ask-Me-Anything) 등 community 이벤트 일정이 잡혀 있다는 정보 — 텔레그램 채널 기반, peaq 공식 트위터/블로그에서는 별도 공지 미확인 상태이므로 일정 정확도는 채널 직접 확인 권장. catalysts(가격 호재 — 이벤트가 가격을 미는 동력) 누적 가능성 있음. ▶ 시장 미리 반응? 지난 7일 +59.6%, 24h +27.6% 강한 상승 진행 중. MastChain(peaq 위 해양 DePIN 프로젝트) MastNode 출하 일정 진행 중(정확한 수량·일정은 peaq 공식 채널 확인 권장) + Bithumb 신규 KRW 상장(VANA 동시) 등 catalyst 후보가 겹친 상태 (개별 catalyst 의 가격 영향은 공식 확인 후 판단 권장). ATL $0.01220(2026-03-15) 대비 현재 $0.02781 = +128% 반등. ▶ 시장 상황은? MCap $57.7M / FDV $120.9M(CoinGecko, 현재 total supply 기준) = 약 2.1배. max supply 기준으로 보면 $154.5M(CMC) = 약 2.7배 — 잠재 희석 압력 중간 수준. ATH $0.7505(2024-12-10) 대비 -96.3%로 깊은 약세 구간이지만 단기 30일 강한 반등. Binance Futures(PEAQUSDT) OI 약 $1.93M(CoinGlass)로 무난한 수준이라 강제청산 리스크 낮음. ▶ 한국에서 팔 수 있나? Bithumb PEAQ/KRW 상장으로 한국 직접 매도 가능. Upbit 미상장. Binance 현물 미상장(Futures만), 해외 현물은 Gate.io·Bitvavo·Bybit이 주력. ▶ 데이터 소스: Tokenomist(unlock 일자·수량·카테고리) + CoinGecko(시장 데이터) + peaq 공식 docs.peaq.xyz(allocation) + CoinGlass(Futures OI) 크로스체크.",
    strategy: [
      { type: "caution", title: "7일 +57.6% 급등 후 6/12 unlock — sell-the-news 위험 우선 인식",
        desc: "지난 7일 +59.6%, 24h +27.6% 강한 상승 진행 중인 상태에서 6/12 unlock(84.84M = 유통량 약 4.0%, $2.36M)이 예정. 급등 후 이벤트가 발생하면 sell-the-news(호재 후 차익 실현 — 기대감으로 미리 산 자금이 이벤트 발생 시 일제히 매도) 패턴 위험이 가장 직접적 시나리오. Core Contributors 카테고리 자체는 즉시 매도보다 운영비·그랜트 분배 비중이 크지만, 차익 실현 매물과 합쳐지면 단기 압력 발생 가능. 사용자 제보 기준 6/11까지 한국 텔레그램 AMA 등 community 일정이 있다고 알려졌으나 peaq 공식 채널(트위터·블로그·docs)에서는 별도 공지 미확인 상태 — catalyst로 가정하기 전 공식 출처 직접 확인 권장.",
        risk: "한국 일정 정보가 미확인 상태인 점이 가장 큰 리스크 — AMA가 실제로 없거나 무관한 채널 정보일 경우 catalyst 가정이 깨지고 시나리오 자체가 흔들림. 또한 DePIN 섹터 동조 반등(IoTeX 등)이나 MastChain 출하 모멘텀이 unlock 매물을 흡수하면 추세 지속 가능 — 한 방향 베팅보다는 6/10~6/11 가격 흐름과 AMA 공식 공지 여부를 확인한 뒤 의사결정." },
    ],
  },
  {
